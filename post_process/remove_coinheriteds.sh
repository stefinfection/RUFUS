...
# static vars
CONTROL_VCF="isec_control.$FMTD_REGION.vcf.gz"
...
ISEC_OUT_DIR="temp_${FMTD_REGION}_isecs"
...
MERGED_PILEUP="merged_pileup.${FMTD_REGION}.vcf.gz"
SORTED_MERGED_PILEUP="sorted.merged_pileup.${FMTD_REGION}.vcf.gz"
...
for CONTROL in "${CONTROL_BAM_LIST[@]}"; do
	...
	CONTROL_ALIGNED="temp_aligned.${CONTROL_TAG}.bam"
	...
	if [ "$WINDOW_SIZE" -ne 0 ]; then
		...
		TEMP_ARGS_FILE="$FMTD_REGION.arguments.txt"
...
rm -f ./*."$FMTD_REGION".ctrl.merged_pileup.vcf.gz*
...
rm -f temp_aligned.*.${FMTD_REGION}.bam
rm -rf "$ISEC_OUT_DIR"